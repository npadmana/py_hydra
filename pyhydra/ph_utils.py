...
def minfilt(x=None,L=None):

    '''
    a simple minimum filter for 1d numpy arrays.

    Based on medfilt up above
    ...
    for i,xi in enumerate(xin):
        ...
        # left boundary (Lwing terms)
        if i < Lwing:
            xout[i] = np.min(xin[0:i+Lwing+1]) # (0 to i+Lwing)

        # right boundary (Lwing terms)
        elif i >= N - Lwing:
            xout[i] = np.min(xin[i-Lwing:N]) # (i-Lwing to N-1)
            
        # middle (N - 2*Lwing terms; input vector and filter window overlap completely)
        else:
            xout[i] = np.min(xin[i-Lwing:i+Lwing+1]) # (i-Lwing to i+Lwing)
